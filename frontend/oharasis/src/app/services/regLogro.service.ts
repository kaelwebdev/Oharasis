...
export class RegLogroService {

  itemClt:AngularFirestoreCollection<RegLogro>;
  item: Observable<RegLogro[]>;
  itemDoc: AngularFirestoreDocument<RegLogro>;
  collectionName:string= 'RegLogros';
  ...
  constructor(public afs:AngularFirestore) {
      ...
      map(
        changes=> {
          ...
          return changes.map( a=> {
            const data = a.payload.doc.data() as RegLogro;
   ...
   addItem(item:RegLogro):Promise<boolean>{
   ...
   deleteItem(item:RegLogro):Promise<boolean>{
   ...
   updateItem(item:RegLogro):Promise<boolean>{
    this.itemDoc = this.afs.doc(`${this.collectionName}/${item.d_id}`);
   ...
   getRegistroV1(a:string, b:number){
    this.itemClt = this.afs.collection(this.collectionName, ref=> ref.where(
      "d_idEstudiante", "==", a).where("idLogro","==",b));
    return this.getMap();
  }
}
