...
export class RegEstudianteService {

  itemClt:AngularFirestoreCollection<RegEstudiante>;
  item: Observable<RegEstudiante[]>;
  itemDoc: AngularFirestoreDocument<RegEstudiante>;
  collectionName:string= 'RegEstudiante';
  ...
  constructor(public afs:AngularFirestore) {
      ...
      map(
        changes=> {
          return changes.map( a=> {
            const data = a.payload.doc.data() as RegEstudiante;
   ...
   addItem(item:RegEstudiante):Promise<boolean>{
   ...
   deleteItem(item:RegEstudiante):Promise<boolean>{
   ...
   updateItem(item:RegEstudiante):Promise<boolean>{
    this.itemDoc = this.afs.doc(`${this.collectionName}/${item.d_id}`);
