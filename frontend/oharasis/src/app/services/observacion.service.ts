...
import { Observacion } from "../../app/interfaces/observacion.interface";
...
export class ObservacionService {
  ...
  itemClt:AngularFirestoreCollection<Observacion>;
  item: Observable<Observacion[]>;
  itemDoc: AngularFirestoreDocument<Observacion>;
  collectionName:string= 'Observaciones';
  ...
  constructor(public afs:AngularFirestore) {
      ...
      map(
        changes=> {
          ...
          return changes.map( a=> {
            const data = a.payload.doc.data() as Observacion;
   ...
   addItem(item:Observacion):Promise<boolean>{
   ...
   deleteItem(item:Observacion):Promise<boolean>{
   ...
   updateItem(item:Observacion):Promise<boolean>{
    this.itemDoc = this.afs.doc(`${this.collectionName}/${item.d_id}`);
   ...
   getRegistroV1(a:string, b:string){
    this.itemClt = this.afs.collection(this.collectionName, ref=> ref.where(
      "d_idCurso", "==", a).where("d_idReceptor","==",b));
    return this.getMap();
  }
